...
DOCKER_APACHE_PHP_CONTAINER_NAME=easyng_apache_php
...
run-bash: ## run bash for apache_php make run-bash CONTAINER=<service>
	docker exec -it $(DOCKER_APACHE_PHP_CONTAINER_NAME) /bin/bash

run-bash-as-www-data:
	docker exec -it --user www-data $(DOCKER_APACHE_PHP_CONTAINER_NAME) /bin/bash

docker-rm-all: ## remove all container and images
...
doctrine-migrate: ## run doctrine migrations
	docker exec -it $(DOCKER_APACHE_PHP_CONTAINER_NAME) bash -c "/var/www/current/vendor/bin/doctrine-migrations migrate"

docker-debug: ## debug docker-compose.yml file
